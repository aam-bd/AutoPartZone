...
  const navLinks = [
    { name: "Home", path: "/" },
    { name: "Products", path: "/shop" }, // **FIXED: Changed from /products to /shop**
    { name: "About Us", path: "/about" },
    { name: "Contact", path: "/contact" },
  ];

  // Prevent "flicker" while checking if user is logged in
  if (loading) return <div className="h-20 bg-white border-b w-full fixed top-0 z-50" />;
...
            <img className="h-12 w-auto transition-transform group-hover:scale-105" src={logo} alt="Logo" />
...
      {/* --- MOBILE MENU --- */}
      <div className={`md:hidden absolute w-full bg-white shadow-xl border-t transition-all duration-300 ease-in-out overflow-hidden ${isMenuOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="px-4 pt-4 pb-6 space-y-2">
          {navLinks.map((link) => (
            <NavLink key={link.name} to={link.path} onClick={() => setIsMenuOpen(false)} className={({ isActive }) => `block px-4 py-3 rounded-lg text-base font-medium ${isActive ? 'bg-red-50 text-[#90281F]' : 'text-gray-700 hover:bg-gray-50'}`}>
              {link.name}
            </NavLink>
          ))}
...
                  <img src={user.photoURL || noprofile} className="h-10 w-10 rounded-full object-cover" alt="Profile" />
                  <div>
                    <p className="text-sm font-bold">{user.displayName}</p>
                    <p className="text-xs text-gray-500">{user.email}</p>
                  </div>
                </div>
                <button onClick={handleLogout} className="p-2 text-gray-500 hover:text-[#90281F]"><LogoutIcon /></button>
              </div>
            ) : (
              <NavLink to="/login" onClick={() => setIsMenuOpen(false)} className="block w-full text-center py-3 rounded-lg font-bold text-white" style={{ backgroundColor: brandColor }}>
                Login Account
              </NavLink>
            )}
          </div>
